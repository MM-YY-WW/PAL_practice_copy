...
def call_gpt(prompt, model='code-davinci-002', stop=None, temperature = 0, top_p=1.0,
    ...
    for i in range(20* (num_completions // num_completions_batch_size + 1)):
        try:
            ...
            #to check if string start with the input string eg: start with 'gpt-4'
...
def completions_api(model, max_tokens, stop, prompt, temperature, top_p, n, best_of):
...
def chat_api(model, max_tokens, stop, prompt, temperature, top_p, n, best_of):
    ans = openai.ChatCompletion.create(
        ...
        stop=stop,
        messages=[
            {'role': 'system', 'content': 'You are a helpful assistant that can write Python code that solves mathmatical reasoning questions similarly to the examples that you will be provided.'},
