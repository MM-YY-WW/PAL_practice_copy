...
class TextInterface:
    def __init__ (
        ...
        self.history = []
        self.answer_predix = answer_prefix
...
class ProgramInterface:
    ...
    def execute(self, code: Optional[List[str]] = None):
        #[Question] self.code does not define
        code = code if code else self.code 
        if self.get_answer_from_stdout:
            program_io = io.StringIO()
            with redirect_stderr(program_io):
...
class ProgramChatInterface(ProgramInterface):
    ...
    def process_generation_to_code(self, gens: str):
        if '```python' in gens:
            gens = gens.aplit('```python')[1].split('```')[0]
        elif '```' in gens:
