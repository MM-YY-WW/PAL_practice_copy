# [Question] line 25: will it better if says for i in range(len(tmp))?
...
REPEAT_COPY_PROMPT = '''
Q: say java twice and data one, and then repeat all of this three times.
...
def solution():
   """Q: ask a group of insects in whta family? four times. after the fourth time say The happy family"""
   result = []
   tmp = []
   for i in range(1,5):
...
Q: Repeat the word duck four times, but halway through also say quack

# solution using Python:

def solution():
    """Q: Repear the word duck four times, but halfway through also say quack"""
    result = []
    for i in range(1, 5):
        result.append("duck")
        if i == 2: 
...
Q: Print boolen eleven times, but after the 3rd and 8th also say correct
...
def solution():
    """Q: I have a chair, two potatoes, a cauliflower, a lettuce head, two tables, a cabbage, two onions, and three fridges. HOw many vegatables do I have?"""
    ...
    num_vegerables = sum(vegerables_to_count.values())
    return num_vegerables
...
def solution():
    """Q: I have a raspberry, a cat, a rabbit, a mouse, a pig, two snails, a fish, two cows, a snake, a goat, and a duck. How many animals do I have? """
    # note: I'm not counting the raspberry as animal
    animals_to_count = {
        'cat': 1,
        'rabbit': 1,
        'mouse': 1,
